...
const groupSessionsByDate = (sessions: SesionsByIdResponse[]): GroupedSessions => {
...
const groupSessionsByCinema = (sessions: SesionsByIdResponse[]): GroupedSessions => {
  const grouped: GroupedSessions = {};
...
const filterPastSessions = (sessions: SesionsByIdResponse[]): SesionsByIdResponse[] => {
  const now = new Date();
...
export default function MovieDetailsClient({ movie, sessions, cinemas }: MovieDetailsClientProps) {
  const posterUrl = getMoviePosterUrl(movie.posterImage);
  const upcomingSessions = filterPastSessions(sessions);
  const groupedSessions = upcomingSessions.length > 0 ? groupSessionsByDate(upcomingSessions) : null;
