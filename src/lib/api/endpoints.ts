...
import { Movie } from "@/types/movie";
  ...
  export const getSessionsById = async (movieId: string): Promise<SesionsByIdResponse[]> => {
  ...
  export const getCinemaSessions = async (cinemaId: string): Promise<CinemaSession[]> => {
  ...
  export const getMovieSession = async (sessionId: number | string): Promise<SessionInfo> => {
    const { data } = await apiClient.get(`/movieSessions/${sessionId}`);
    return data;
  };

  export const bookSeats = async (sessionId: string, bookingData: BookingRequest): Promise<BookingResponse> => {
    const token = localStorage.getItem('authToken');
    const { data } = await apiClient.post(`/movieSessions/${sessionId}/bookings`, bookingData, {
      headers: {
...
export const payForBooking = async (bookingId: string): Promise<void> => {
  const token = localStorage.getItem('authToken');
  const { data } = await apiClient.post(`/bookings/${bookingId}/payments`,{ bookingId} , {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
